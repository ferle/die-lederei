/*
  # Add email settings functionality

  1. Changes
    - Add email_settings JSONB column to settings table
    - Add default email settings configuration
    - Create email_queue table for reliable email sending
    - Add RLS policies for email queue
    - Create triggers for order and registration confirmation emails
...
DROP POLICY IF EXISTS "Allow admin access to email queue" ON email_queue;
...
-- Create or replace function to send order confirmation email
...
-- Create or replace function to send registration confirmation email
